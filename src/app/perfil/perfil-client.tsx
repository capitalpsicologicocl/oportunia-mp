...
export function PerfilClient() {
  ...
  useEffect(() => {
    Promise.all([
    ...
    ]).then(([orgData, authData, , notifData, backlogData]) => {
      ...
      if (notifData.notifications) setMentions(notifData.notifications);
  ...
  async function updateNote(id: string, patch: Partial<Pick<StickyNote, "title" | "body" | "color">>) {
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6">
            ...
            {notes.map((note) => {
              ...
              return (
                <div
                  key={note.id}
                  className={cn(
                    "brand-card relative flex min-h-[140px] flex-col border-2 p-4 shadow-sm transition-shadow hover:shadow-md",
                  ...
                  {isEditing ? (
                    <div className="flex flex-1 flex-col gap-2 pt-4">
                      <Input
                        ...
                        className="bg-white/60 font-medium"
                        placeholder="Título"
                      />
                      <textarea
                        value={note.body}
                        onChange={(e) => updateNote(note.id, { body: e.target.value })}
                        className="min-h-[72px] flex-1 resize-none rounded-lg border border-input bg-white/60 px-2.5 py-2 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50"
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="xs" onClick={() => setEditingNoteId(null)}>
                          <X className="size-3" />
                          Listo
                        </Button>
                      </div>
      ...
      <section className="brand-card space-y-4 p-6">
        <div className="flex items-center justify-between">
          <h3 className="font-heading text-lg font-semibold text-[#11233d]">Menciones recientes</h3>
          <Link href="/bandeja" className="text-sm text-[#d4a017] hover:underline">
            Ver bandeja
          </Link>
        </div>
        {mentions.length === 0 ? (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{formatFechaCL(mention.created_at)}</p>
                  </div>
                  {!mention.leida && (
                    <span className="rounded-full bg-[#d4a017] px-2 py-0.5 text-[10px] font-medium text-[#11233d]">
                      Nueva
                    </span>
                  )}
                </div>
                {mention.kanban_card_id && (
                  <Link
                    href={`/kanban?card=${mention.kanban_card_id}`}
